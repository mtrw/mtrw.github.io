...
#HERE IS WHERE I AM UP TO WRITING!!! PESTER ME TO FINISH THE REST!!!

Do_something can get very complex. Use curly braces (`{` and `}`) to denote multi-command operations in there. I speculate that a driver's fastest lap of a race is usually a few laps before the last lap, when they are pushing the car hard but the tyres have not yet worn out too much. Let's look at our current world champion Max Verstappen and check how many laps from the end of the race he completed his fastest lap.

```{r}
ltA[,{
  lastLap <- max(lap)
  verstappenFastest <- .SD[fullname=="Max Verstappen",lap[seconds==min(seconds)]]
