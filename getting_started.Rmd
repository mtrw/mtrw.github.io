...
#### Operating systems and shells and things

There's a little terminology to learn here.
...
Linux is a text-based operating system, meaning you instruct it what to do by typing commands into a prompt, rather than the familiar visual way of doing things in Windows or MacOS. It sounds scary but it's actually incredibly advantageous for bioinformatics, because you can ask it to do much more complex and interesting things than you can easily do in Windows (especially when you are dealing with millions of very large files).

For example, if I want to move every one of ten thousand files that begin with the letter "A" from `directory1` to `directory2`, this would be hard in Windows. In Linux we just type ...

```
mv directory1/A* directory2/
```

... and it is done instantly.

Rather than  A *teletypewriter* (or *TTY*) interface is a device that provides a screen and a keyboard so you can use this text interface.
...
*PuTTy* uses the *SSH* *protocol* to connect to a *server* and provide a *TTY* interface to the Zsh *shell* around the server's *operating system* ... which is *Linux*.
...
1. Get PuTTy from [here](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) and install it.

2. Get WinSCP from [here](https://winscp.net/download/WinSCP-6.5.6-Setup.exe/download) and install it.
...
To get access to the Spartan server, you need to join CropGEM's *project*. Ask Tim to send you a request, and you'll get an email at your unimelb address. Another email will arrive to confirm you are a member and give you a username, usually identical to your UoM login. The CropGEM project is called "punim1869".
...
Go on to step 8 just below under "*Both Windows and MacOS users*".
...
3. Storage and your workspace
    - The directory you land in is called your *home directory*. It is basically meant for things like profile files. It does not have much disk space, so do NOT do project work in your home directory. Seriously. **DO NOT DO PROJECT WORK IN YOUR HOME DIRECTORY**.
    
    Instead, we have a lot of allocated storage space for project work. It is located in subdirectories under `/data/gpfs/projects/punim1869/`.
        ...
        - *Alternatively* alternatively, there is a variable `$STO` set up containing the address, so you can also use (e.g. `cd $STO`).
    - A workspace has been set up for you here. It is in `users/YOUR_USERNAME`. By default, no one else can access it. Use it for ... everything.
        - You can get there with the alias `gw` ("[g]o to my [w]orkspace").
...
5. What now?
    ...
    - Learn to [transfer data between Spartan and your own computer](./tutorial_sftp.html), including how to set up SFTP Drive.
    ...
    - Do you want to use R studio on spartan? Read about [how to get set up with R](./tutorial_Rstart.html), [the basics of using R](./tutorial_R.html), and how to [become a fierce data analyst with data.table](./tutorial_data_table.html).
    - 
