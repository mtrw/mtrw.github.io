...
However, in Linux programs will often output things to either *stdout* (which generally prints to the screen if not redirected to a file; it is designed for program output) or *stderr* (which prints on a different "channel", and hence can be redirected separately to stdout; it is designed for error messages). Once your job is complete, any output to these will be given in two files `job_<job-name>.out` and `job_<job-name>.err`.
