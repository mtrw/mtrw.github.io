...
# ... even if it contains legitimate commands like 1+1
1+1 # Produces 2
```
...
In R, you end up typing the same things a lot. The IDE (Rstudio) will guess the ending of a lot of half-typed things, when prompted by the `<TAB>` key. For example, we know the variable 'letters' exists. Just type ...

```{r,eval=F}
le
```

..., leave your cursor at the end and hit `<TAB>`. You will get a scrollable, clickable, list of possible autocompletes:
...
v < 0
v == 0
v != 1 # != means "is not equal to". Many programming languages associate `!` with "not"
v_notEqual_zero <- v != 0 # We can save the results in a variable
print(v_notEqual_zero)
```
...
And one last thing, the very common function `which()` tells you **which** entries of a logical vector are TRUE. So `which(c(TRUE,FALSE,TRUE))` will return the vector ( 1 , 3 ), because 1 and 3 are the indices of the TRUE entries. Based on this, explain to yourself why these produce the same result:
...
# Look, I know you are thinking "actually ... that seems pointless". And yes, which() is often pointless. But it is extremely common, and so you need to know it to read other peoples' code.
```
