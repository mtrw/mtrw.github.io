...
This is where we use the free program [SFTP Drive 2024](https://www.callback.com/sftpdrive/download), Which implements a [s]ecure [f]ile [t]ransfer [p]rotocol. STFP is basically a set of rules governing how computers can securely transfer data between them. There are command line programs that implement SFTP, but SFTP Drive applies some sexy wrapping to make it operate just like a regular directory on your own computer!

First, install SFTP drive from [here](https://www.callback.com/sftpdrive/download).

Then open SFTP Drive 2024 and click <new>.

![](images/drive1.png)

Then set up a connection to Spartan. First, make a connection to your home directory. Just fill it out and check boxes as shown below where underlined, the rest will fill automatically. You can enter any drive name you want, and use your Spartan username and password.
...
To add a drive that directly takes you to storage, simply repeat the process and configure the drive like this, with the drive as (e.g.) /data/gpfs/projects/punim1869/users/\<YOUR_USERNAME\>:
...
So, say I am on Spartan and I want to copy a directory **/home/tim/data/seqfiles** from the server at my old work (where my username is **wallace**, the server address is **ipk-gatersleben.de**, and my password is of course a secret) I would run:
